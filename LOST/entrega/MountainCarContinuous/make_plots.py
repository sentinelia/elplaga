...
# Language support: `python make_plots.py --lang es` writes Spanish-labeled
# figures to results/es/ (used by the Spanish submission materials).
LANG = "es" if "--lang" in os.sys.argv and "es" in os.sys.argv else "en"
OUT = os.path.join(RESULTS, "es") if LANG == "es" else RESULTS

ES = {
    "Return (moving avg over 100 episodes)": "Retorno (media m\u00f3vil de 100 episodios)",
    "Training episode": "Episodio de entrenamiento",
    "State-grid resolution (4 actions, mean of 2 seeds)": "Resoluci\u00f3n de la grilla de estados (4 acciones, promedio de 2 semillas)",
    "Action-set size (20x20 grid, mean of 2 seeds)": "Tama\u00f1o del conjunto de acciones (grilla 20x20, promedio de 2 semillas)",
    "Learning rate alpha": "Tasa de aprendizaje \u03b1",
    "Discount gamma": "Factor de descuento \u03b3",
    "Epsilon decay": "Decaimiento de \u00e9psilon",
    "Epsilon floor": "\u00c9psilon m\u00ednimo",
    "Hyperparameter sweeps around the base configuration (* = base value)": "Barridos de hiperpar\u00e1metros alrededor de la configuraci\u00f3n base (* = valor base)",
    "Eval return (100 greedy episodes)": "Retorno de evaluaci\u00f3n (100 episodios greedy)",
    "Dyna-Q planning budget vs. Q-Learning (600 episodes, mean of 2 seeds)": "Planificaci\u00f3n de Dyna-Q vs. Q-Learning (600 episodios, promedio de 2 semillas)",
    "Eval return after 600 episodes": "Retorno de evaluaci\u00f3n tras 600 episodios",
    "Training wall-clock time": "Tiempo de entrenamiento (reloj de pared)",
    "Seconds": "Segundos",
    "Return (moving avg)": "Retorno (media m\u00f3vil)",
    "State value  max_a Q(s, a)": "Valor del estado  max_a Q(s, a)",
    "Greedy throttle (blue = push left, red = push right)": "Aceleraci\u00f3n greedy (azul = izquierda, rojo = derecha)",
    "Position": "Posici\u00f3n",
    "Velocity": "Velocidad",
    "goal": "meta",
    "success": "\u00e9xito",
...
def tr(s):
    return ES.get(s, s) if LANG == "es" else s
...
def curves_figure(groups, title, path, ylabel=None):
    ...
    ax.set_xlabel(tr("Training episode"))
    ax.set_ylabel(ylabel or tr("Return (moving avg over 100 episodes)"))
    ax.set_title(title)
    ax.legend(loc="lower right", fontsize=9)
    ax.margins(x=0.12)
    fig.savefig(os.path.join(OUT, path))
    plt.close(fig)
...
def fig_discretization():
    ...
    curves_figure(
        [(f"{'grilla ' if LANG == 'es' else ''}{k}{'' if LANG == 'es' else ' grid'}", [r["run_id"] for r in v]) for k, v in by_grid.items()],
        tr("State-grid resolution (4 actions, mean of 2 seeds)"),
    ...
    curves_figure(
        [(f"{k} {'acciones' if LANG == 'es' else 'actions'}", [r["run_id"] for r in v]) for k, v in sorted(by_act.items())],
        tr("Action-set size (20x20 grid, mean of 2 seeds)"),
...
def fig_hyperparams():
    ...
    sweeps = {
        "alpha": (tr("Learning rate alpha"), "0.1"),
        "gamma": (tr("Discount gamma"), "0.99"),
        "epsilon_decay": (tr("Epsilon decay"), "0.999"),
        "epsilon_min": (tr("Epsilon floor"), "0.05"),
    }
    ...
    for ax, (param, (title, base_val)) in zip(axes.flat, sweeps.items()):
        ...
        ax.set_xlabel(tr("Eval return (100 greedy episodes)"))
    fig.suptitle(tr("Hyperparameter sweeps around the base configuration (* = base value)"), y=1.0)
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "fig3_hyperparams.png"))
    plt.close(fig)
...
def fig_dynaq():
    ...
    curves_figure(
        ...
        tr("Dyna-Q planning budget vs. Q-Learning (600 episodes, mean of 2 seeds)"),
    ...
    bars_figure(ax1, labels, rewards, rerrs, SERIES[0], tr("Eval return after 600 episodes"))
    ax1.set_xlabel(tr("Eval return (100 greedy episodes)"))
    bars_figure(ax2, labels, times, [0] * len(times), SERIES[1], tr("Training wall-clock time"))
    ax2.set_xlabel(tr("Seconds"))
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "fig5_dynaq_tradeoff.png"))
    plt.close(fig)
...
def fig_final():
    ...
    for ax, r, color in zip(np.atleast_1d(axes), rows, SERIES):
        ...
        ax.set_title(
            f"{r['run_id']}\n{tr('eval')} {float(r['eval_mean_reward']):.1f} "
            f"({float(r['eval_success_rate']) * 100:.0f}% {tr('success')})",
            fontsize=9,
        )
        ax.set_xlabel(tr("Training episode"))
        ax.set_ylabel(tr("Return (moving avg)"))
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "fig6_final_learning_curves.png"))
    plt.close(fig)
...
def fig_policy():
    ...
    ax1.set_title(tr("State value  max_a Q(s, a)"))
    ...
    ax2.set_title(tr("Greedy throttle (blue = push left, red = push right)"))
    fig.colorbar(im2, ax=ax2, shrink=0.85)
    for ax in (ax1, ax2):
        ax.set_xlabel(tr("Position"))
        ax.set_ylabel(tr("Velocity"))
        ax.grid(False)
        ax.axvline(0.45, color=TEXT_SECONDARY, lw=1, ls="--")
        ax.annotate(tr("goal"), xy=(0.45, 0.06), fontsize=8, color=TEXT_SECONDARY, ha="right")
    fig.tight_layout()
    fig.savefig(os.path.join(OUT, "fig7_policy_value.png"))
    plt.close(fig)
